...
arg00(){
  ...
  NAMENODE=hadoop
  CONTAINER=Hadoop
  docker run --name ${HADOOP_CONTAINER_NAME} -h ${NAMENODE} \
  --net znet --ip ${NET}.${IP} \
  -e NODES=$NODES \
  $HOSTS \
  -p 9001:9001 \
...
#  NAMENODE=hadoop
#  CONTAINER=Hadoop
#  docker run --rm --name ${HADOOP_CONTAINER_NAME} -h ${NAMENODE} \
#  --net znet --ip ${NET}.${IP} \
#  -e NODES=$NODES \
#  $HOSTS \
#  -p 8088:8088 \
...
Stop_remove(){
  ...
  # Stop namenode
#  CONTAINER=Hadoop
  docker stop ${HADOOP_CONTAINER_NAME}
  docker rm ${HADOOP_CONTAINER_NAME}
  # Stop Mariadb
#  CONTAINER=MariaDB
...
Start(){
  ...
  # Start namenode
  CONTAINER=Hadoop
  docker start ${HADOOP_CONTAINER_NAME}
#  # Start Mariadb
#  CONTAINER=MariaDB
...
Restart(){
  ...
  # Restart namenode
  CONTAINER=Hadoop
  docker restart ${HADOOP_CONTAINER_NAME}
  # Restart Mariadb
#  CONTAINER=MariaDB
